...
if (savedZoom) {
    map.setView(savedCenter, Number(savedZoom));
}
...
async function loadHistory() {
    const response = await fetch(`${API}/iss/history`);
    ...
    const history = data.slice(start);
    pathSegments = [[]];
    pathSegments[0] = history.map(p => [p.latitude, p.longitude]);

    line.setLatLngs(pathSegments);
    marker.setLatLng(pathSegments[0][pathSegments[0].length - 1]);
}
...
async function loadAPOD() {
    ...
    if (data.image_url.endsWith(".mp4") || data.image_url.includes("youtube")) {
        media = `
            <video class="apod-img" controls>
                <source src="${data.image_url}">
            </video>
        `;
    } else {
        media = `
            <img class="apod-img" src="${data.image_url}" alt="${data.title}">
        `;
    }
...
async function loadAsteroids() {
    const statsResponse = await fetch(`${API}/asteroids/stats`);
    ...
    document.getElementById("stats").innerHTML = `
        Total: ${stats.total} | Dangerous: ${stats.hazardous} | ${stats.start_date} → ${stats.end_date}
    `;

    const unique = Array.from(new Map(data.map(a => [a.key, a])).values());
    const closest = unique.sort((a, b) => new Date(a.date) - new Date(b.date)).slice(0, 10);
    ...
    closest.forEach(a => {
        rows += `
            <tr>
                <td>${a.name}</td>
                <td>${a.diameter}m</td>
                <td>${a.miss_distance.toLocaleString()}km</td>
                <td>${a.velocity.toLocaleString()}km/h</td>
                <td>${a.date}</td>
                <td class="${a.hazardous ? "hazard" : "safe"}">
                    ${a.hazardous ? "YES" : "NO"}
                </td>
            </tr>
        `;
    ...
    document.getElementById("asteroids-table-body").innerHTML = rows;

    createVelocityChart(closest);
    createDistanceChart(closest);
}

function createVelocityChart(data) {
    new Chart(document.getElementById("asteroidsChart"), {
        type: "bar",
        data: {
            labels: data.map(x => x.name),
            datasets: [{
                label: "Velocity (km/h)",
                data: data.map(x => x.velocity),
                backgroundColor: data.map(x => x.hazardous ? "#ff4444" : "#66ccff")
            }]
        },
        options: {
            ...
            scales: {
                x: {
                    ...
                    grid: { color: "#404b63", display: false }
                },
                y: {
                    beginAtZero: true,
                    min: 0,
                    ticks: { color: "#b0b0b0", maxTicksLimit: 6 },
                    border: { display: true, color: "#404b63" },
                    grid: { color: "#404b63" }
                }
            }
        }
...
function createDistanceChart(data) {
    new Chart(document.getElementById("distanceChart"), {
        ...
        data: {
            ...
            datasets: [{
                label: "Distance (km)",
                data: data.map(x => x.miss_distance),
                borderColor: "#66ccff",
                pointBackgroundColor: data.map(x => x.hazardous ? "#ff4444" : "#66ccff"),
                pointRadius: 6
            }]
        },
        options: {
